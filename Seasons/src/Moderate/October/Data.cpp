...
void Data::setByteAt(unsigned int position, byte value) {
	if (position >= theSize)
		throw OutOfBoundsException();

	buffer[position] = value;
}
void Data::setBytesAt(unsigned int position, byte value[], unsigned int count) {
	if (position + count >= theSize)
		throw OutOfBoundsException();
	memcpy(&buffer[position], value, count);
}
void Data::resize(int newSize) {
	if (newSize != theSize) {
		byte * newBuffer = new byte[newSize + 1];
		memcpy(newBuffer, buffer, theSize);
		delete buffer;
		buffer = newBuffer;
		theSize = newSize;
	}
